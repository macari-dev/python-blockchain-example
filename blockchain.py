...
class Blockchain:
                ...
                for node in self.__peer_nodes:
                    ...
                    try: 
                        ...
                        if response.status_code == 400 or response.status_code == 500:
                            print('transaction resolve, needs resolving')
                            return False
    ...
    def add_block(self, block):
        transacactions = [Transaction(tx['sender'], tx['recipient'], tx['signature'], tx['amount']) for tx in block['transactions']]
        proof_is_valid = Verification.valid_proof(transacactions[:-1], block['previous_hash'], block['proof'])
        hashes_match = hash_block(self.chain[-1]) == block['previous_hash']
        if not proof_is_valid or not hashes_match:
            return False
        converted_block = Block(block['index'], block['previous_hash'], transacactions, block['proof'], block['timestamp'])
        self.__chain.append(converted_block)
        stored_transactions = self.__open_transactions[:]
        for itx in block['transactions']:
            for otx in stored_transactions:
